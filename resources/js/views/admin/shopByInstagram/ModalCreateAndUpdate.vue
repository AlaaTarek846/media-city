        ...
        <div class="modal-dialog modal-lg modal-dialog-scrollable">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="row">
                         <div class="col-md-12">
                            <label for="link_path" class="form-label">{{$t('global.link')}}</label>
                            <input type="text" class="form-control" id="link_path" :placeholder="$t('global.link')"
                                v-model.trim="v$.link_path.$model"
                                :class="{'is-invalid': v$.link_path.$error ||errors[`link_path`],
                                'is-valid':!v$.link_path.$invalid && !errors[`link_path`]}">
                            <div class="invalid-feedback">
                                <span v-if="v$.link_path.required.$invalid">{{ $t('validation.fieldRequired') }}<br /> </span>
                            </div>
                            <template v-if="errors['link_path']">
                                <error-message v-for="(errorMessage, index) in errors['link_path']" :key="index">
                                    {{ errorMessage }}
                                </error-message>
                            </template>
                        </div>
...
export default {
       ...
       function resetModal() {
            ...
            setTimeout(async () => {
                if (props.type != 'edit') {
                    ...
                    submitdata.data.link_path = props.dataRow.link_path;
    ...
    methods: {
        ...
        formData.append('link', this.data.link_path);
        if (this.image) {
        ...
        if (this.type !== 'edit') {
            if (!this.v$.$error && this.numberOfImage) {
                ...
                this.loading = true;
                adminApi.post(`dashboard/shop-by-instagram`, formData)
                    .then((res) => {
                        Swal.fire({
                            icon: 'success',
            ...
            adminApi.post(`dashboard/shop-by-instagram/${this.id}`,formData)
                .then((res) => {
                    Swal.fire({
                        icon: 'success',
