<template>
    <div>
            ...
            <h1 class="page-title fw-semibold fs-18 mb-0">{{ $t('global.shopByInstagram') }}</h1>
            <div class="ms-md-1 ms-0">
                <nav>
                    <ol class="breadcrumb mb-0">
                        <li class="breadcrumb-item"><router-link :to="{name: 'dashboard'}">{{$t('global.home')}}</router-link></li>
                        <li class="breadcrumb-item active" aria-current="page">{{ $t('global.shopByInstagram') }}</li>
                    </ol>
                </nav>
            </div>
        </div>
                ...
                <div class="card custom-card">
                    <div class="card-header justify-content-between">
                        ...
                        <div class="prism-toggle">
                            <button v-if="permission.includes('shop by instagram create')" @click="showModelCreate" class="btn btn-sm btn-primary-light" data-bs-toggle="modal" data-bs-target="#banner-modal">
                        ...
                        <div class="table-responsive mb-2">
                            <table class="table text-nowrap table-striped">
                                ...
                                <tr v-for="(item,index) in data" :key="item.id">
                                    ...
                                    <td>{{item.link_path}}</td>
                                    <td>
                                        <div class="hstack gap-2 fs-15">
                                            <button v-if="permission.includes('shop by instagram edit')"
                                                @click.prevent="showEditMode(item)"
                                                data-bs-toggle="modal" data-bs-target="#banner-modal"
                                               class="btn btn-icon btn-sm btn-info-transparent rounded-pill"><i
                                                class="ri-edit-line"></i></button>
                                            <a href="#" @click.prevent="deleteData(item.id,index)" v-if="permission.includes('shop by instagram delete')"
