...
export default {
    name: "product-attributes-modal-create-and-update",
    props: {
        type: {default: 'create'},
        dataRow: {default: ''},
    },
    ...
    setup(props){
       ...
       function resetModal() {
            ...
            setTimeout(async () => {
                ...
                adminApi.get(`dashboard/product-attributes/${id.value}`)
                    .then((res) => {
                        loading.value = true;
                        let l = res.data.data;
       ...
       this.languages.forEach((el)=>{
        ...
        if (this.type !== 'edit') {
            if (!this.v$.$error) {
                ...
                this.loading = true;
                adminApi.post(`dashboard/product-attributes`, formData)
                    .then((res) => {
                        Swal.fire({
                            icon: 'success',
            ...
            adminApi.post(`dashboard/product-attributes/${this.id}`,formData)
                .then((res) => {
                    Swal.fire({
                        icon: 'success',
