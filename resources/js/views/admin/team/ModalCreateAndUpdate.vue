            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="row">
                        ...
                        <div class="col-md-6" v-if="data.ar" v-for="lang in languages">
                            <label class="form-label">{{ $t('global.job_title') }} ({{lang.title}})</label>
                            <textarea
                        ...
                        <div class="col-md-6 mt-3">
                            <label class="form-label">{{$t('global.facebook')}}</label>
                            <div class="input-group">
                                <input type="text" class="form-control" :placeholder="$t('global.facebook')"
                        ...
                        <div class="col-md-6 mt-3">
                            <label class="form-label">{{$t('global.twitter')}}</label>
                            <div class="input-group">
                                <input type="text" class="form-control" :placeholder="$t('global.twitter')"
                                       v-model.trim="v$.x.$model"
                                       :class="{'is-invalid': v$.x.$error ||errors[`x`],
                                        'is-valid':!v$.x.$invalid && !errors[`x`]}">
                            </div>
                            <template v-if="errors['x']">
                                <error-message v-for="(errorMessage, index) in errors['x']" :key="index">
                                    {{ errorMessage }}
                                </error-message>
                            </template>
                        </div>
                        <div class="col-md-6 mt-3">
                            <label class="form-label">{{$t('global.instagram')}}</label>
                            <div class="input-group">
                                <input type="text" class="form-control" :placeholder="$t('global.instagram')"
       ...
       function resetModal() {
            ...
            setTimeout(async () => {
                if (props.type != 'edit') {
                    ...
                    adminApi.get(`dashboard/team/${id.value}`)
                    .then((res) => {
                        loading.value = true;
                        let l = res.data.data;
    ...
    methods: {
        ...
        if (this.type !== 'edit') {
            if (!this.v$.$error && this.numberOfImage) {
                ...
                this.loading = true;
                adminApi.post(`dashboard/team`, formData)
                    .then((res) => {
                        Swal.fire({
                            icon: 'success',
            ...
            adminApi.post(`dashboard/team/${this.id}`,formData)
                .then((res) => {
                    Swal.fire({
                        icon: 'success',
