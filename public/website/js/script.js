...
(function(){
  ...
  function updateTotal(){
    const d = Math.max(1, parseInt(durationVal.value || 1, 10));
    const u = durationUnit.value;
    const q = Math.max(1, parseInt(qty.value || 1, 10));
    const total = perDay * toDays(u) * d * q;
    totalEl.textContent = total.toFixed(2);
  }

  priceUnitSel.addEventListener('change', updateUnitPrice);
  durationVal.addEventListener('input', updateTotal);
  durationUnit.addEventListener('change', updateTotal);
...
(function(){
  ...
  const backHeaderBtn = document.getElementById('waBackHeader');

  const main = document.getElementById('waMain');
  const openFormBtn = document.getElementById('waOpenForm');
  const openWaBtn = document.getElementById('waOpenWhatsApp');

  const formWrap = document.getElementById('waFormWrap');
  const submitBtn = document.getElementById('waSubmit');

  const nameEl = document.getElementById('waName');
  const emailEl = document.getElementById('waEmail');
  ...
  const number = (panel.dataset.waNumber || '').replace(/\D/g,'');
  const company = panel.dataset.companyName || 'Support';
  const defaultMsg = panel.dataset.defaultMessage || 'Hello';

  const headerTitle = panel.querySelector('.wa-header b');
  if(headerTitle) headerTitle.textContent = `تواصل مع ${company}`;

  function openPanel(){ panel.classList.add('open'); }
  function closePanel(){
    panel.classList.remove('open');
    showMain(); // يرجّع للوضع الأساسي
  }

  function resetErrors(){
    [errName, errEmail, errPhone].forEach(e => e.classList.remove('show'));
  }

  function showMain(){
    panel.classList.remove('mode-form');
  ...
  function openWhatsApp(message){
    if(!number){
      alert('من فضلك حط رقم واتساب صحيح في data-wa-number');
      return;
    }
  ...
  // زر الـ Back في الهيدر
  backHeaderBtn.addEventListener('click', showMain);
  ...
  openWaBtn.addEventListener('click', () => openWhatsApp(defaultMsg));
  ...
  submitBtn.addEventListener('click', () => {
    ...
    let ok = true;
    if(!name){ errName.classList.add('show'); ok = false; } else errName.classList.remove('show');
    if(!isValidEmail(email)){ errEmail.classList.add('show'); ok = false; } else errEmail.classList.remove('show');
    if(phone.length < 7){ errPhone.classList.add('show'); ok = false; } else errPhone.classList.remove('show');
    if(!ok) return;
